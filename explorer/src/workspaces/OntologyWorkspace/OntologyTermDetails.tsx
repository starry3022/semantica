...
const formFields = (term: OntologyTermSnapshot["term"]): Fields => ({ ...term, parents: term.parents.join("\n"), domain: term.domain.join("\n"), range: term.range.join("\n") });
  ...
  const save = async () => {
    ...
    try {
      const result = await saveOntologyTerm(ontologyUri, termUri, {
        ...
        parents: listValue(draft.parents), domain: listValue(draft.domain), range: listValue(draft.range),
      });
  ...
  return <section aria-label="Term definition" style={{ color: "#ebf3ff", fontSize: 13, lineHeight: 1.5 }}>
    ...
    {snapshot ? <>
      ...
      {draft ? <form onSubmit={(event) => { event.preventDefault(); void save(); }}>
        ...
        {isClass ? field("parents", "Parent class IRIs") : <>{field("domain", "Domain class IRIs")}{field("range", "Range IRIs")}</>}
        <p style={{ color: "#8fa8c6", fontSize: 12 }}>One IRI per line. Class and XSD datatype references must already be loaded. {isClass ? "" : "Shared property edits affect all classes using this property."}</p>
        ...
        {isClass ? definition("Parent classes", snapshot.term.parents.join("\n")) : <>{definition("Domain", snapshot.term.domain.join("\n"))}{definition("Range", snapshot.term.range.join("\n"))}</>}
