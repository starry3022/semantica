  ...
  return (
    <section style={sectionStyle} aria-label="Source material & evidence">
      ...
      {error ? <div role="alert" style={mutedStyle}>{error}</div> : !view ? (
        ...
        <>
          <div style={mutedStyle}>{view.sources.length} source material{view.sources.length === 1 ? "" : "s"} · {view.evidence.length} evidence item{view.evidence.length === 1 ? "" : "s"}</div>
    ...
    <div style={backdropStyle}>
      <div
        ...
        onKeyDown={(event) => {
          ...
          const controls = dialogRef.current?.querySelectorAll<HTMLElement>('button, select, [tabindex="0"]');
      ...
      >
        <header style={{ display: "flex", gap: 16, alignItems: "flex-start", justifyContent: "space-between" }}>
          <div>
            <h2 id={titleId} style={{ fontSize: 19, margin: "0 0 8px", color: GRAPH_THEME.ui.text.strong }}>Source material & evidence</h2>
            <div style={{ ...mutedStyle, overflowWrap: "anywhere" }}>{view.selection.label || view.selection.id}</div>
          </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 20, minHeight: 0 }}>
          <aside style={{ flex: "1 1 230px", minWidth: 0, display: "flex", flexDirection: "column", gap: 12 }} aria-label="Evidence list">
          ...
          <div style={{ flex: "3 1 450px", minWidth: 0, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <section style={{ ...sectionStyle, gap: 6 }} aria-label="Source identity">
              ...
              <div style={metadataStyle}>Source ID: {source?.source_id || selectedEvidence?.source_id || "Unknown"}</div>
              <div style={metadataStyle}>Version: {source?.version || "Unknown"}</div>
              {source?.source_uri ? <div style={metadataStyle}>Source URI: {source.source_uri}</div> : null}
              <div style={metadataStyle}>Expected SHA-256: {source?.source_sha256 || selectedEvidence?.source_sha256 || "Unknown"}</div>
              <div style={metadataStyle}>Actual SHA-256: {source?.actual_sha256 || "Unknown"}</div>
...
const dialogStyle: CSSProperties = { display: "flex", flexDirection: "column", gap: 20, width: "min(1160px, 100%)", maxHeight: "calc(100dvh - 32px)", overflow: "auto", padding: 24, borderRadius: 16, border: `1px solid ${GRAPH_THEME.ui.control.activeBorder}`, background: "#111c1d", boxShadow: "0 24px 100px rgba(0,0,0,0.5)" };
...
const sourceTextStyle: CSSProperties = { margin: 0, padding: 20, minHeight: 200, maxHeight: "48vh", overflowY: "auto", whiteSpace: "pre-wrap", overflowWrap: "anywhere", lineHeight: 1.9, fontSize: 14, fontFamily: "inherit", color: GRAPH_THEME.ui.text.strong, borderRadius: 10, border: `1px solid ${GRAPH_THEME.ui.surface.panelBorder}`, background: "#0b1415" };
